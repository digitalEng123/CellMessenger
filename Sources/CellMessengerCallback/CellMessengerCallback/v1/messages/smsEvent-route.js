...
function getSMSEventRoute(app) {
    ...
    route.get('/smsEvent', function (request, response, next) {

        response.end("GEt Not Supported!");
    });
};

function postSMSEventRoute(app) {
    ...
    /* GET users phone numbers. */
    route.post('/smsEvent', function (request, response, next) {
                            ...
                            "select top 1 * from dbo.tblMailDetail where intRecordID_Mail = " + intRecordID_Mail, function (error, mailDetailRecordSet) {
                                ...
                                if (!utilities.IsUndefined(mailDetailRecordSet) && mailDetailRecordSet.length > 0) {
                                    ...
                                    Promise.all(promises).then(function () {
                                        ...
                                        if (direction === 'in') {
                                            ixtCode = "Inbound SMS Event from " + from;
                                        }
                                        else {
                                            ixtCode = "Outbound SMS Event to " + to;
                                        }
...
module.exports = {
    RegisterGetSMSEventRoute: getSMSEventRoute,
    RegisterPostSMSEventRoute: postSMSEventRoute
};